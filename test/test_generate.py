...
def test_pbs_dependencies_array_only():
    scripts = generate_scripts(spec(scheduler="pbs", depend_array_ids=["9[]"]))
...
def test_slurm_dependency_ids():
    scripts = generate_scripts(spec(scheduler="slurm", depend_job_ids=["11", "22"]))
...
def test_individual_chunk_slicing():
    tasks = [f"echo {i}\n" for i in range(4)]
    scripts = generate_scripts(
        spec(scheduler="sge", task_list=tasks, chunk_size=2, individual=True)
    )
...
def test_array_script_body():
    tasks = [f"echo {i}\n" for i in range(4)]
    scripts = generate_scripts(
        spec(scheduler="sge", task_list=tasks, chunk_size=2, cores="2")
    )
    ((name, text),) = scripts
...
def test_env_copied_uses_spec_environ_not_os_environ():
    scripts = generate_scripts(
        spec(scheduler="pbs", env="copied", environ={"MYVAR": "myvalue"})
    )
    text = scripts[0][1]
...
def test_env_copied_skips_ignored_vars():
    scripts = generate_scripts(
        spec(
            scheduler="pbs", env="copied", environ={"PWD": "/somewhere", "KEEP": "yes"}
        )
    )
    text = scripts[0][1]
...
def test_cores_as_int_reaches_the_script():
    scripts = generate_scripts(
        spec(scheduler="sge", cores=4, ppj=4, task_list=["a\n", "b\n"], chunk_size=1)
    )
